...
export class Assessment {
  ...
  async next(id: string, user: string) {
    ...
    const messages = (await this.diagnosis.messages(id,user)).map(x=>x.payload);
    const result = await this.ai.run('A-03',id,InterviewSchema,'Ask ONE useful, observational follow-up question based on the actual answers. Do not ask the user to touch, open, disconnect, test electrical components or approach hazards. Stop when sufficient information exists. diy_guide=remote_batteries only when a handheld remote has been identified, weak/empty batteries are likely, and the user explicitly confirmed a dry, intact, cool battery compartment with no corrosion, leakage or swelling. Otherwise none.',{messages,safety,visual_assessments:row.payload.visual_assessments,related_record_ids:row.payload.related_record_ids},{next_question:null,hypotheses:[],confidence:0,information_gaps:['AI assessment unavailable'],enough_information:false,category:'general',diy_guide:'none'});
