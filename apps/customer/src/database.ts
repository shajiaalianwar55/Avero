...
export interface Database { list(table: string, filters?: Record<string, string>): Promise<Row[]>; save(table: string, row: Row): Promise<Row>; }
...
export async function owned(db: Database, table: string, id: string, userId: string) {
  const row = (await db.list(table, { id, user_id: userId }))[0];
  if (!row) throw new HttpError(404, 'Record not found'); return row;
}
